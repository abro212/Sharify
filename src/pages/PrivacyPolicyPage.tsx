  ...
  useEffect(() => {
    ...
    const fetchLogo = async () => {
      ...
      try {
        ...
        if (!cancelled) {
          if (error) {
            console.error('[PrivacyPolicyPage] Logo fetch error:', error.message);
            setLogoUrl(null);
          } else {
        ...
        if (!cancelled) {
          console.error('[PrivacyPolicyPage] Unexpected logo fetch error:', err);
          setLogoUrl(null);
        }
      } finally {
          ...
          <div className="flex items-center space-x-4">
            ...
            <div>
              ...
              <p className="text-slate-400 text-xs sm:text-sm font-semibold mt-1">
                Perlindungan data dan privasi pengguna sesuai dengan nilai-nilai amanah.
              </p>
            </div>
          </div>
