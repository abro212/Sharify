...
export const SignUp: React.FC = () => {
  ...
  const onSubmit = async (data: any) => {
    ...
    if (error) {
      ...
      setSuccessMsg('Pendaftaran berhasil! Silakan periksa email Anda untuk verifikasi akun.');
    ...
    <div className="min-h-screen bg-gradient-to-br from-emerald-50 via-white to-amber-50/40 flex flex-col justify-center py-12 sm:px-6 lg:px-8 relative overflow-hidden font-sans">
      ...
      <div className="sm:mx-auto sm:w-full sm:max-w-md relative z-10 px-4">
        ...
        <p className="mt-2 text-center text-sm text-gray-500">
          Wujudkan pengelolaan finansial masa depan yang berkah dan syariah
        </p>
      </div>
      ...
      <div className="mt-8 sm:mx-auto sm:w-full sm:max-w-md relative z-10 px-4">
        <div className="bg-white/80 backdrop-blur-md py-8 px-6 sm:px-10 shadow-2xl border border-white/50 rounded-3xl">
          <form className="space-y-5" onSubmit={handleSubmit(onSubmit)}>
            ...
            {successMsg && (
              <div className="bg-emerald-50 text-emerald-600 border border-emerald-100 p-3.5 rounded-xl text-xs font-semibold text-center">
                {successMsg}
              </div>
            )}
            ...
            <div>
              ...
              {errors.fullName && <p className="mt-1 text-xs text-rose-600 font-medium">{errors.fullName.message as string}</p>}
            </div>
          ...
          <div className="mt-6">
            <div className="relative">
              ...
              <div className="relative flex justify-center text-xs uppercase">
                <span className="bg-white/80 px-3.5 text-gray-400 font-bold tracking-wider text-[10px]">Atau mendaftar dengan</span>
              </div>
            </div>
          ...
          <div className="mt-8 pt-5 border-t border-gray-100/60 text-center">
            <p className="text-xs text-gray-500">
              Sudah punya akun?{' '}
              <Link to="/login" className="font-extrabold text-emerald-700 hover:text-emerald-950 hover:underline transition-all">
                Masuk Sekarang
              </Link>
            </p>
          </div>
